...
defineAriaSource('options.label', (ctx) => ctx.options?.label)
defineAriaSource('options.orientation', (ctx) => ctx.options?.orientation)
defineAriaSource('options.roledescription', (ctx) => ctx.options?.roledescription)
defineAriaSource('options.slideRoledescription', (ctx) => ctx.options?.slideRoledescription)
...
defineAriaSource('items.valuemin', (ctx) => (ctx.key ? ctx.data.items[ctx.key]?.valuemin : undefined))
defineAriaSource('items.valuemax', (ctx) => (ctx.key ? ctx.data.items[ctx.key]?.valuemax : undefined))
defineAriaSource('items.valuetext', (ctx) => (ctx.key ? ctx.data.items[ctx.key]?.valuetext : undefined))
defineAriaSource('options.min', (ctx) => ctx.options?.min)
defineAriaSource('options.max', (ctx) => ctx.options?.max)
defineAriaSource('options.haspopup', (ctx) => ctx.options?.haspopup ?? 'listbox')
defineAriaSource('options.autocomplete', (ctx) => ctx.options?.autocomplete ?? 'list')
defineAriaSource('state.rowCount', (ctx) => ctx.data.state?.rowCount ?? ctx.data.relations?.rowKeys?.length)
defineAriaSource('state.colCount', (ctx) => ctx.data.state?.colCount ?? ctx.data.relations?.columnKeys?.length)
