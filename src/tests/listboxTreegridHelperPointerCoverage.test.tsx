...
function ListboxTreegridHelperHost() {
  ...
  const listboxKeyEvents = useRef<PatternEvent[]>([])
  const multiListbox = useListboxPattern(
    {
      items: { a: { label: 'Alpha' } },
      relations: { rootKeys: ['a'] },
      state: { activeKey: 'a', selectedKeys: [] },
    },
    (event) => listboxKeyEvents.current.push(event),
  ...
  return (
    <div>
      <button
        type="button"
        onClick={() => {
          listboxKeyEvents.current = []
          const eventBase = { shiftKey: true, ctrlKey: false, metaKey: false, preventDefault: () => undefined }
          multiListbox.rootProps.onKeyDown?.({ ...eventBase, key: 'ArrowDown', code: 'ArrowDown' } as never)
          multiListbox.rootProps.onKeyDown?.({ ...eventBase, key: 'ArrowUp', code: 'ArrowUp' } as never)
          multiListbox.rootProps.onKeyDown?.({ ...eventBase, key: 'Home', code: 'Home', ctrlKey: true } as never)
          multiListbox.rootProps.onKeyDown?.({ ...eventBase, key: 'End', code: 'End', ctrlKey: true } as never)
          setResult(listboxKeyEvents.current.map((event) => event.type === 'select' ? event.keys.join(',') : event.type).join('|') || 'none')
        }}
      >
        Run listbox keys
      </button>
