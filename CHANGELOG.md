# Changelog

## 0.1.0

- Initial publish-ready package surface for APG pattern definitions, runtime helpers, and React adapters.
- Root and `./core` entries are React-free; React hooks and preset components are exposed from `./react`.
- Package verification covers exports, packed files, side effects, ESM/CJS runtime imports, README Quick Start type checks, npm tarball installation, TypeScript consumer imports, and demo smoke tests.
