...
- Project identity starts from keyboard behavior contracts: APG is the reference language for shared keyboard, focus, active cursor, and semantic event expectations.
...
- Interaction ownership experiments may incubate in `packages/interaction` and `docs/proposals`, but must stay out of the public `@interactive-os/aria` runtime surface until the boundary is stable.
